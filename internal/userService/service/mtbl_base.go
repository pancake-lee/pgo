...
type MtblFieldConfig struct {
	Col     *papitable.AddField
	DOField string
}

type MtblTableConfig struct {
	TableName  string
	PrimaryCol *papitable.AddField
...
type BaseMtblHandler struct {
	...
	M2LFunc   func(mtblRecord *papitable.CommonRecord, localRecord any) any
	GetIDFunc func(record any) int32
}

// impl papitable.DataProvider
func (h *BaseMtblHandler) GetTableName() string {
	return h.TableConfig.TableName
}

// impl papitable.DataProvider
func (h *BaseMtblHandler) GetPrimaryCol() *papitable.AddField {
	return h.TableConfig.PrimaryCol
}

// impl papitable.DataProvider
func (h *BaseMtblHandler) GetColList() []*papitable.AddField {
	cols := make([]*papitable.AddField, len(h.TableConfig.ColList))
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) SetDoc(doc *papitable.MultiTableDoc) {
	h.Doc = doc
}

// impl papitable.DataProvider
func (h *BaseMtblHandler) L2M(record any, oldMtblRecord *papitable.CommonRecord) map[string]any {
	if h.L2MFunc != nil {
	...
	for _, fieldCfg := range h.TableConfig.ColList {
		...
		case papitable.FIELD_TYPE_TEXT:
			valMap[fieldCfg.Col.Name] = papitable.NewTextValue(fmt.Sprintf("%v", fieldVal.Interface()))
		case papitable.FIELD_TYPE_NUMBER:
			...
			valMap[fieldCfg.Col.Name] = papitable.NewNumValue(num)
		}
	}
	return valMap
}
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) GetSyncData() ([]*papitable.AddRecord, error) {
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) GetLastEditFrom(record any) string {
	if record == nil {
		return ""
	}
	val := reflect.ValueOf(record)
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) UpdateMtblRecordID(id any, mtblRecordId, lastEditFrom string) error {
	return nil
}

// impl papitable.DataProvider
func (h *BaseMtblHandler) M2L(mtblRecord *papitable.CommonRecord, localRecord any) any {
	if h.M2LFunc != nil {
	...
	for _, fieldCfg := range h.TableConfig.ColList {
		...
		switch fieldCfg.Col.Type {
		case papitable.FIELD_TYPE_TEXT:
			strVal, _ := papitable.ParseTextValue(v)
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) GetLocalRecordByMtbl(mtblRecord *papitable.CommonRecord) (any, error) {
...
// impl papitable.DataProvider
func (h *BaseMtblHandler) UpdateLocalRecord(localRecord any, mtblRecord *papitable.CommonRecord) (newRecord any, stop bool, err error) {
...
func (h *BaseMtblHandler) HandleMtblEvent() error {
...
func (h *BaseMtblHandler) deleteM2L(recordId string) error {
	papitable.LastEditFromMarker_LTBL.Set(recordId)
...
func (h *BaseMtblHandler) updateM2L(datasheetId, recordId string) error {
	...
	syncHelper := papitable.NewSyncHelper(h, doc).
