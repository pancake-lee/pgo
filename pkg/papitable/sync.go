...
func (s *SyncHelper) InitMTBL(spaceId, datasheetId string) (string, error) {
	...
	if len(rows) > 0 {
		newRows, err := doc.AddRow(rows)
		if err != nil {
			s.log.Errorf("AddRow failed: %v", err)
			return datasheetId, err
		}
		for _, newRow := range newRows {
			LastEditFromMarker_MTBL.Set(newRow.RecordId)
		}
...
func (s *SyncHelper) UpdateToMTBL(
	...
	if LastEditFromMarker_LTBL.SkipAndClean(localId, updateTime) {
		s.log.Debugf("Local data [%s] is recently[%v] edited for [LastEditFrom], skip update", localId, updateTime)
		return nil
	}

	s.log.Debugf("Local data %s edited[%v], now sync to MTBL", localId, updateTime)

	localLastEditFrom := s.dataProvider.GetLastEditFrom(localRecord)

	oldRecord, _ := s.getMtblRecordByLocalId(localId)
	...
	if localLastEditFrom != LastEditFrom_TEMP {
		if oldRecord == nil {
			...
			if fieldList == nil {
				s.log.Errorf("local L2M [%v] is nil, skip", localId)
				return nil
			}
	...
	err = s.dataProvider.UpdateLastEditToLTBL(localId, newLastEditFrom)
	if err != nil {
	...
	s.log.Debugf("Local rewrite [%v] LastEditFrom[%v]",
		localId, newLastEditFrom)
...
func (s *SyncHelper) DeleteMTBL(localId string, deleteTime time.Time) error {
	...
	if LastEditFromMarker_LTBL.SkipAndClean(localId, deleteTime) {
		s.log.Debugf("Local data [%s] is recently[%v] delete by MTBL, skip", localId, deleteTime)
		return nil
	}
	s.log.Debugf("Local data %s edited[%v], now sync to MTBL", localId, deleteTime)
...
func (s *SyncHelper) UpdateToLTBL(mtblRecord *CommonRecord) error {
	...
	// Check Loop Marker
	if LastEditFromMarker_MTBL.SkipAndClean(mtblRecordId, time.Unix(mtblRecord.UpdatedAt/1000, 0)) {
		s.log.Debugf("MTBL recordId %s is recently[%v] edited for [LastEditFrom], skip update", mtblRecordId, time.Unix(mtblRecord.UpdatedAt/1000, 0))
		return nil
	}
	s.log.Debugf("MTBL recordId %s edited[%v], now sync to LTBL", mtblRecordId, time.Unix(mtblRecord.UpdatedAt/1000, 0))
	// Get Local Record
	...
	// 注意这里修复了双方id的一致性问题：以MTBL的recordId为索引，把LTBL的id写回MTBL
...
func (s *SyncHelper) getMtblRecordByLocalId(localId string,
	...
	if len(resp.Data.Records) == 0 {
		return nil, fmt.Errorf("local data %s not found in mtbl", localId)
