...
func newUserJobHandler() *gotest.Handler {
	...
	d.IDao = dao.NewUserJobDao(d.DB, c.ICache.(cache.UserJobCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &userJobHandler{iDao: d.IDao.(dao.UserJobDao)}
	iHandler := h.IHandler.(UserJobHandler)
...
func Test_userJobHandler_Create(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := &types.CreateUserJobRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.UserJob))
...
func Test_userJobHandler_DeleteByID(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
...
func Test_userJobHandler_UpdateByID(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := &types.UpdateUserJobByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.UserJob))
...
func Test_userJobHandler_GetByID(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
...
func Test_userJobHandler_List(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListUserJobsRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func Test_userJobHandler_DeleteByIDs(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteUserJobsByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteUserJobsByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_userJobHandler_GetByCondition(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("GetByCondition"), &types.GetUserJobByConditionRequest{
		query.Conditions{
...
func Test_userJobHandler_ListByIDs(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("ListByIDs"), &types.ListUserJobsByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = httpcli.Post(result, h.GetRequestURL("ListByIDs"), &types.ListUserJobsByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_userJobHandler_ListByLastID(t *testing.T) {
	h := newUserJobHandler()
	defer h.Close()
	testData := h.TestData.(*model.UserJob)
...
func TestNewUserJobHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewUserJobHandler()
