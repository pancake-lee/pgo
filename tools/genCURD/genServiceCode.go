...
func genServerCode(
	...
	logger.Debugf("gen svr code for svr[%v]", dstSvcName)
...
func genServiceCodeForOneTable(
	...
	logger.Debugf("gen svr code for tbl[%v]", tbl.Model.TableName())
	...
	// --------------------------------------------------
	importPkgList = util.StrListUnique(importPkgList)
	importCode := ""
...
func svcNameReplace(codeStr, tplSvcName, dstSvcName string) string {
	...
	codeStr = strings.ReplaceAll(codeStr,
		util.StrToCamelCase(tplSvcName)+"CURDServer",
		util.StrToCamelCase(dstSvcName)+"CURDServer")

	codeStr = strings.ReplaceAll(codeStr,
		util.StrToCamelCase(tplSvcName)+"CURDHTTPServer",
		util.StrToCamelCase(dstSvcName)+"CURDHTTPServer")
